...
generate_data <- function(way = "linear", setting_seed = 1, noise_seed = -1){
  if(way == "linear"){
    ...
    X <- gene_X("X_AR_Strong", n, p, setting_seed, scaling = F, model_X = T)$X  # "IID_Normal", "MCC_Block", "X_AR_Strong" 
...
setting_seed <- 1
data <- generate_data(way = "linear", setting_seed, noise_seed = -1)
X <- data$X
y <- data$y
H0 <- data$H0
beta <- data$beta
...
error.samples <- sapply(1:100, function(mc_i){
  data.mc <- generate_data(way = "linear", setting_seed, noise_seed = -mc_i)
  X.mc <- data.mc$X
  y.mc <- data.mc$y
  H0.mc <- data.mc$H0
...
tic()
hFDR.obj <- hFDR_on_cv(cv.obj, model = "fixed-X", n_tune = ifelse(method=="FS",n_tune,40), n_cores = 16)
toc()

### draw figures. use show_cv, show_hFDR, show_FPR to control whether to show cv curve, hFDR, FDR/FDP if provided, and FPR/FPP. See hFDR.R > plot.hFDR

single.aspect.ratio <- 1.4
single.width <- 6
single.mar <- c(4,4,3,4)

triple.aspect.ratio <- 1.4
triple.width <- 4.5
triple.mar <- c(4,4,2,0.5)
triple.legend <- FALSE
triple.axes <- FALSE

## Baseline scenario

scenario = "Correlated X"  ## "Baseline", "Correlated X", "Forward Stepwise"

pdf(file = here("figs","talk","intro",gsub(" ","_",scenario),"single_noFDR.pdf"), width = single.width, height = single.width/single.aspect.ratio)
par(mar = single.mar, cex = 1, cex.axis = 1, cex.lab = 1)
plot(hFDR.obj, cv.obj, errors, sign.tune = 2*(method %in% c("FS"))-1,
     show_cv = T, show_hFDR = F, show_FPR = F, show_FDR = F, log_cv = F, leg_bty="n")
dev.off()

pdf(file = here("figs","talk","intro",gsub(" ","_",scenario),"single_FDR_nohFDR.pdf"), width = single.width, height = single.width/single.aspect.ratio)
par(mar = single.mar, cex = 1, cex.axis = 1, cex.lab = 1)
plot(hFDR.obj, cv.obj, errors, sign.tune = 2*(method %in% c("FS"))-1,
     show_cv = T, show_hFDR = F, show_FPR = F, log_cv = F, leg_bty="n")
dev.off()

pdf(file = here("figs","talk","intro",gsub(" ","_",scenario),"single_hFDR.pdf"), width = single.width, height = single.width/single.aspect.ratio)
par(mar = single.mar, cex = 1, cex.axis = 1, cex.lab = 1)
plot(hFDR.obj, cv.obj, errors, sign.tune = 2*(method %in% c("FS"))-1,
...
pdf(file = here("figs","talk","intro",gsub(" ","_",scenario),"triple_noFDR.pdf"), width = triple.width, height = triple.width/triple.aspect.ratio)
par(mar = triple.mar, cex = 1, cex.axis = 1, cex.lab = 1)
plot(hFDR.obj, cv.obj, errors, sign.tune = 2*(method %in% c("FS"))-1,
     show_cv = T, show_hFDR = F, show_FPR = F, show_FDR = F, log_cv = F, 
     show_legend=triple.legend, show_extra_axes = triple.axes, main = scenario)
dev.off()

pdf(file = here("figs","talk","intro",gsub(" ","_",scenario),"triple_FDR_nohFDR.pdf"), width = triple.width, height = triple.width/triple.aspect.ratio)
par(mar = triple.mar, cex = 1, cex.axis = 1, cex.lab = 1)
plot(hFDR.obj, cv.obj, errors, sign.tune = 2*(method %in% c("FS"))-1,
     show_cv = T, show_hFDR = F, show_FPR = F, log_cv = F, leg_bty="n", 
     show_legend=triple.legend, show_extra_axes = triple.axes, main = scenario)
...
pdf(file = here("figs","talk","intro",gsub(" ","_",scenario),"triple_hFDR.pdf"), width = triple.width, height = triple.width/triple.aspect.ratio)
par(mar = triple.mar, cex = 1, cex.axis = 1, cex.lab = 1)
plot(hFDR.obj, cv.obj, errors, sign.tune = 2*(method %in% c("FS"))-1,
     show_cv = T, show_hFDR = T, show_FPR = F, log_cv = F, leg_bty="n", 
     show_legend=triple.legend, show_extra_axes = triple.axes, main = scenario)
...
# lasso.cv <- cv.glmnet(X, y, nfolds = 10, lambda = tune_seq,
